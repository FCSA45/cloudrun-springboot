...
public class H5LoginService {
    ...
    public LoginUser login(String code) {
        ...
        if (R.isError(userInfoR)) {
            String message = StringUtils.isNotEmpty(userInfoR.getMsg()) ? userInfoR.getMsg() : "用户服务返回异常";
            throw new ServiceException(message);
        }
        UserInfo userInfo = userInfoR.getData();
        if (userInfo == null) {
            log.warn("wxLogin success response but data is null, response msg: {}", userInfoR.getMsg());
