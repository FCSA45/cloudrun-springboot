...
public class OrderInfoApiController extends BaseController
{
    ...
    @Operation(summary = "根据订单号获取订单详细信息")
    @RequiresLogin
    @GetMapping(value = "/getByOrderNo/{orderNo}")
    public R<OrderInfo> getByOrderNoForLoginUser(@PathVariable("orderNo") String orderNo)
    {
    ...
    @Operation(summary = "内部-根据订单号获取订单信息")
    @InnerAuth
    @GetMapping("/internal/getByOrderNo/{orderNo}")
    public R<OrderInfo> getByOrderNoInternal(@PathVariable String orderNo) {
        OrderInfo orderInfo = orderInfoService.getByOrderNo(orderNo);
        return R.ok(orderInfo);
    }
