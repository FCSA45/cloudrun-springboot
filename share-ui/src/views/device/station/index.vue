...
import { listStation, getStation, delStation, addStation, updateStation, setData } from "../../../api/device/station.js";
import { searchNoUseList } from "../../../api/device/cabinet.js";
import { getTreeSelect } from "../../../api/device/region.js";
import { getALLFeeRuleList } from "../../../api/rule/feeRule.js";
...
function handleUpdate(row) {
  ...
  if (!_id || (Array.isArray(_id) && _id.length === 0)) {
    proxy.$modal.msgWarning("请选择要修改的数据项");
    return;
  }
...
function handleDelete(row) {
  ...
  if (!_ids || (Array.isArray(_ids) && _ids.length === 0)) {
    proxy.$modal.msgWarning("请选择要删除的数据项");
    return;
  }
